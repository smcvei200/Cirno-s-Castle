...
LRESULT CALLBACK cWNDManager::WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		{
			rotationAngle -= 90; //Increase rotation Angle
		...
		{
			rotationAngle += 90; //Decrease rotation Angle
			//glRotatef(rotationAngle, 0.0f, 1.0f, 0.0f);
			if (turns == 3)
				turns = 0;
			else
		...
		if (wParam == 'C') //If the Down Arrow key was pressed
		{
			ortho = true;
		}
		if (wParam == 'V') //If the Down Arrow key was pressed
		{
			ortho = false;
		}
		if (wParam == 'S')
